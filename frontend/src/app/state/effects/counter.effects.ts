  ...
  loadCounterPrefs$ = createEffect(
    () => {
      return this.actions$.pipe(
        ...
        map((storedStuff) => JSON.parse(storedStuff!)), // { count: 1, by: 3} as CounterState - need to validate (using Zod)
        map(
        ...
        catchError((err) => {
          return of(
            applicationEvents.error({ message: 'no haxxing the localstorage' })
          );
        })
      );
    },
